...
void print_help() {
	...
	printf(
			...
			"  - ALTSERVER_NO_SUBSCRIBE: set (any value, even 0) to skip the usbmuxd device-event\n"
			"          subscription. Nothing polls instead: requests look the device up when they\n"
			"          run. The subscription only tracks USB devices, so with netmuxd (network\n"
			"          devices only) it does nothing and can be turned off.\n"
